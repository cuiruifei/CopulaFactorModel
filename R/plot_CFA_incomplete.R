...
#       weighted least squares (DWLS), and the full information maximum likelihood (FIML) on mixed
#       incomplete for learning parameters of CFA models. 
#
# Metrics: 1) ARB; 2) ARMSE.
#
# Settings: 1) a correlated 4-factor model with specific parameters where each factor has 4 indicators
#           2) mixed nonparanormal (df = 8) and ordinal (4 levels) indicators
#           3) n = 500
#           4) percent of missing values = {0, 0.1, 0.2, 0.3}
########################################################################################################
...
data2stat <- function(data){
  ...
  describe(data) %>%
    ...
    mutate(Methods = c(rep('FIML',4), rep('DWLS',4), rep('BGCF',4))) %>%
    mutate(Sample.size = as.factor(rep(c(0, 0.1, 0.2, 0.3) * 100, 3))) %>%
...
## raw data
results = read.table(file = "results/factor_analysis/k4_mixed_perMissing.txt")
...
ARB.corr = ggplot(stat, aes(x = Sample.size, y = mean, group = Methods, color = Methods)) + coord_cartesian(ylim = c(-0.15, 0.15)) +
  ...
  geom_hline(yintercept = -0.1, linetype = "dotted") +
  xlab('missing percentage') + ylab('ARB') +
  #geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.2, position=pd) + 
  theme_bw() +
  theme(legend.justification=c(0.01,0.99),legend.position=c(0.01,0.99),legend.title = element_blank(), legend.key.size = unit(0.35, "cm")) +
...
RMSE.corr = ggplot(stat, aes(x = Sample.size, y = mean, group = Methods, fill = Methods)) + 
  ...
  xlab('missing percentage') + ylab('RMSE') +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.2, position=pd) + 
  theme_bw() +
...
# ## save Figure
# pdf(file = 'results/factor_analysis/k4_incomplete_corr.pdf', width = 4.5, height = 1.9)
# grid.arrange(ARB.corr, RMSE.corr, nrow = 1, ncol = 2)
# dev.off()
...
ARB.corr = ggplot(stat, aes(x = Sample.size, y = mean, group = Methods, color = Methods)) + coord_cartesian(ylim = c(-0.2, 0.2)) +
  geom_line() + 
  geom_point(aes(shape=Methods)) + 
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = 0.05, linetype = "dashed") +
...
grid.arrange(ARB.corr, RMSE.corr, nrow = 1, ncol = 2)
# ## save Figure
# pdf(file = 'results/factor_analysis/k4_incomplete_loadings.pdf', width = 4.5, height = 1.9)
# grid.arrange(ARB.corr, RMSE.corr, nrow = 1, ncol = 2)
# dev.off()
