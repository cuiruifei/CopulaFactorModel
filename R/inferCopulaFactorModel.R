######################################################################################
## Goal:
#  Main function to perform inference for Gaussian copula factor models.
#
## args: 
# 1) Y, a n by p matrix (n-observations, p-variables);
# 2) Lambda, a p by k matrix where rows and columns represent observed variables 
#    and latent factors respectively.
#
## return:
# A collection of samples of the latent correlation matrix
#
## preconditions:
# 1) S0, a p x p prior covariance matrix (scale matrix for G-Wishart prior)
# 2) n0, an integer hyperparameter (degrees of freedom for G-Wishart prior)
##
# Author: ...
######################################################################################
...
inferCopulaFactorModel <- function (Y, Lambda = diag(ncol(Y)), trueSigma = NULL, ind.cat = integer(0), S0 = diag(dim(Y)[2]), n0 = dim(Y)[2] + 1, nsamp = 100, 
